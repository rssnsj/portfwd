...
int ipv4_rules_cmd_parse(struct ipv4_rules *rules, char *cmd)
{
	...
	if(extra_str && rules->fn_get_edata)
	{
		...
		if(data & IPV4_SEGMENT_FLAGS_MASK)
		{
			fprintf(stderr, "Lowest 2 bits of extra data should be zero, but '0x%lx' obtained.\n", data);
			//BUG();
			return -EFAULT;
		}
	}
...
/**************************************************
 *
 * PART 2: Global IP filter rules
 *
 **************************************************/

//static struct ipv4_rules g_ipf_rules;
//
//bool ipf_check(u32 ip)
//{
//	return  ipv4_rules_check(&g_ipf_rules, ip, NULL);
//}
//int ipf_init(void)
//{
//	return ipv4_rules_init(&g_ipf_rules);
//}
//void ipf_exit(void)
//{
//	ipv4_rules_purge(&g_ipf_rules);
//}
